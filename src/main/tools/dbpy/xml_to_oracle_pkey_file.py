...
$Header: /home/inqwell/cvsroot/dev/scripts/python/xml_to_oracle_pkey_file.py,v 1.1 2009/05/22 22:16:04 sanderst Exp $
$Author: sanderst $
...
def _build_write_sql_section(table_info):
    ...
    text = """        WHEN MATCHED THEN
            UPDATE SET"""
    result_lines.append(text)
    line_format = """                %%-%ds = ?""" % table_info.non_pk_db_col_name_max_size
    lines = []
    for db_col_name in table_info.non_pk_db_col_names:
        lines.append(line_format % db_col_name)
    result_lines.append(",\n".join(lines))
    ...
    text = """            )
            ...
            ("""
    result_lines.append(text)
    result_lines.append(",\n".join(["""                ?"""] * len(table_info.all_db_col_names)))
    text = """            )
        \""""
    result_lines.append(text)
    return "\n".join(result_lines)
...
def _build_write_order_section(table_info):
    """
    Build the "write-order" section of the pkey file.
    @param IN table_info Table information
    @return The section as a string
    """
    ...
    line_format = """            "%s\""""
    lines = []
    for xy_col_name in table_info.pk_xy_col_names:
        lines.append(line_format % xy_col_name)
    for xy_col_name in table_info.non_pk_xy_col_names:
        lines.append(line_format % xy_col_name)
    for xy_col_name in table_info.all_xy_col_names:
        lines.append(line_format % xy_col_name)
    result_lines.append(",\n".join(lines))
    text = """        )"""
    result_lines.append(text)
...
def table_elt_to_pkey_text(table_elt):
    ...
    table_info.pk_xy_col_names = [get_xy_column_name(c) for c in column_elt_list]
    ...
    table_info.non_pk_xy_col_names = [c for c in table_info.all_xy_col_names if c not in
        table_info.pk_xy_col_names]
    table_info.non_pk_db_col_names = [c for c in table_info.all_db_col_names if c not in
        table_info.pk_db_col_names]
    table_info.non_pk_db_col_name_max_size = max([len(db_col_name) for db_col_name in
        table_info.non_pk_db_col_names])
...
auxcfg(map(
    ...
    result_lines.append("%s," % text)
    # - write-order
    text = _build_write_order_section(table_info)
    result_lines.append("%s," % text)
    # - delete-sql
