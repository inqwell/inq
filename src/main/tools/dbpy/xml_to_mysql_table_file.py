...
$Header: /home/inqwell/cvsroot/dev/scripts/python/xml_to_mysql_table_file.py,v 1.1 2009/05/22 22:16:01 sanderst Exp $
$Author: sanderst $
$DateTime: 2009/04/14 16:57:37 $
$Change: 164243 $
"""
...
from xml_to_mysql_utils import get_db_column_type
from xml_to_mysql_utils import get_db_foreign_key_column_target
from xml_to_mysql_utils import get_db_foreign_key_name
from xml_to_mysql_utils import get_db_foreign_key_target_table
from xml_to_mysql_utils import get_db_index_name
from xml_to_mysql_utils import get_db_primary_key_name
from xml_to_mysql_utils import get_db_table_name
from xml_to_mysql_utils import get_db_unique_key_name


class XmlToMySqlTable(XmlToDbTable):
    """
    Class to convert an XML database table file into a file containing corresponding MySQL
    statements.
    """
...
REFERENCES %%s (%%s);""" % table_name
    ...
    def _get_index_format(self, table_name):
        ...
        return """CREATE INDEX %%s ON %s (%%s);""" % table_name
...
def main():
    ...
    parser.add_option("--out", dest="output_file", help="Output MySQL file")
    parser.add_option("--out_dir", dest="output_dir", help="Output directory for MySQL files")
    parser.add_option("--template", dest="template_file", help="Template file for the MySQL files")
