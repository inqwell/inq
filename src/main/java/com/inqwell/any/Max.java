...
/**
 * Compare two Anys and return which ever is the maximum.  The original
 ...
 */
public class Max extends OperatorVisitor
{
  /*
  public Any doOperation (Any op1, Any op2) throws AnyException
  {
    // Overridden so we can check for field ripping
    super.doOperation(op1, op2);
    
    Transaction t = getTransaction();
    
    // We had to resolve two operands but only one of them needs
    if (t.getResolving() == Transaction.R_FIELD)
    {
      var = t.getLastTField();
      if (var != null)
        var = var.bestowConstness();
